...
#include <af/nn/Modules/Convolution2D.hpp>
...
namespace af
{
    namespace nn
    {
      ...
      Convolution2D::Convolution2D(int wx, int wy, int sx, int sy, int px, int py, int n_in, int n_out, bool bias, float spread) :
            m_bias(bias),
            m_wx(wx),
            m_wy(wy),
            m_sx(sx),
        ...
        Convolution2D::Convolution2D(const Variable &w) :
            m_bias(false),
            Module({w})
        {
        }

        Convolution2D::Convolution2D(const Variable &w, const Variable &b) :
            m_bias(true),
            Module({w, b})
        {
